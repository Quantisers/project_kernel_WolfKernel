...
curl -s -X POST https://api.telegram.org/bot$BOT_API_KEY/sendMessage -d text="Beta Build Scheduled for $KERNELNAME Kernel (Treble) in Travis CI..." -d chat_id=$CHAT_ID
${MAKE} $DEFCONFIG;
START=$(date +"%s");
